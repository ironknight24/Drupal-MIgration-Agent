...
1. Read `state/migration-state.yml`:
   ...
   - Summarize component counts across the 15 canonical states (`NOT_STARTED`, `DISCOVERED`, `ANALYZED`, `PLANNED`, `SCAFFOLDED`, `IN_PROGRESS`, `CODE_COMPLETE`, `TESTING`, `TESTS_PASSED`, `VALIDATING`, `VALIDATED`, `COMPLETED`, `BLOCKED`, `BLOCKED_UPSTREAM`, `SKIPPED`) recorded in `component_states`.
...
2. Read `state/migration-manifest.yml`:
   - Summarize static component inventory counts by category (custom modules, contrib modules, themes, configuration, data migrations).
