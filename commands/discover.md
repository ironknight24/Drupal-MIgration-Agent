...
2. Validate that `source.path` exists and does not overlap with `target.path`.
3. Treat both environments as strictly READ-ONLY.
4. Activate the `discovery` agent (`agents/discovery.md` / `agents/discovery/agent.md`) to inspect the systems.
5. Populate `state/migration-manifest.yml` with the discovered component inventory.
