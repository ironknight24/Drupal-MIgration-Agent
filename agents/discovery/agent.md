...
21. **Runtime Behavior, Cache, Session, Security & Lifecycle Discovery (Step 23)**:
    - Recursively scan all D7 files (`.module`, `.inc`, `.php`, `.install`, `.profile`, `.drush.inc`, theme/template files) for cache operations (`cache_get`, `cache_set`, `cache_clear_all`), custom cache bins, cache keys, cache metadata, and invalidation triggers.
    - Discover session usage (`$_SESSION`, `drupal_session_*`, `user_save`), cookie operations (`$_COOKIE`, `setcookie`), temporary storage (`drupal_tempstore`), user state, and request state.
    - Analyze access control (`access callback`, `user_access`, `node_access`, custom access logic), permissions, CSRF tokens (`drupal_get_token`, `drupal_valid_token`), XSS sanitization (`check_plain`, `filter_xss`, `check_url`), file/upload validation, URL/redirect security, and API/webhook security.
