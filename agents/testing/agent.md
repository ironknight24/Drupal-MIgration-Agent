...
- Testing custom database repository services, custom entity CRUD, schema installations, and transaction rollback integrity.
- Testing data migration ETL pipelines (source extraction, process transformations, destination storage, and rollback).
- Formulating component-specific test strategies (Unit/Kernel for custom modules and repositories, Schema validation for config, Twig/CSS linting for themes, Count & Cardinality reconciliation for data migrations).
