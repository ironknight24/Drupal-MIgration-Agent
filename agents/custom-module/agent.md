...
## 2. Purpose
Re-engineers legacy Drupal 7 custom modules, custom PHP classes, interfaces, traits, and `.inc` files into modern, object-oriented Drupal 10/11 modules. Governs the 12-step modernization methodology, consuming the complete `custom_php_files` and `inc_files` discovery inventory, prioritizing business logic preservation, constructor dependency injection, non-1:1 architectural mapping, and clean Symfony/Drupal architecture over mechanical syntax translation.
...
- Consuming the custom PHP class, constructor, `.inc` discovery analysis, include graphs, and caller references produced by `discovery`.
- Authoring module migration plans in `reports/custom-modules/PLAN-<MODULE>.md` with exhaustive file-to-class/function accounting.
...
- Authoring modern PSR-4 OOP classes (`src/Service/`, `src/Controller/`, `src/Form/`, `src/Plugin/`, `src/EventSubscriber/`, `src/Drush/Commands/`, `src/Entity/`).
...
- Proposing component state transitions via `agent_result` with explicit outcome statuses for all files, classes, and callables.
