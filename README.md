...
## Legacy Custom Database, Schema, Data Model, Custom Entities, Bundles, Fields, Revisions, Translations, Forms, Form Alters, AJAX, Frontend JavaScript, CSS, Libraries, Procedural Hooks, Configuration, State, Variables, Legacy Custom PHP File, OOP Class & Legacy `.inc` File Re-engineering Architecture
...
- **Recursive Discovery Without Naming Assumptions**: Scans all custom module roots and nested subdirectories (`includes/`, `lib/`, `classes/`, `src/`, `admin/`, `forms/`, `pages/`, `commands/`, `js/`, `css/`, etc.) discovering all `*.php`, `*.inc`, `*.module`, `*.install`, `*.profile`, `*.drush.inc`, `*.js`, `*.css`, `*.scss`, and `*.less` files.
