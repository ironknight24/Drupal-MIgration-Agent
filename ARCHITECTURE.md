...
## 4. Legacy Custom Database, Schema, Data Model, Procedural Hooks, Legacy Custom PHP File, OOP Class & Legacy `.inc` File Re-engineering Architecture
...
- **Strict Zero-Omission Outcome Accounting**: Every custom database table, schema, data model, procedural hook implementation, custom hook, custom PHP file, class, constructor, method, and `.inc` file must resolve to `MIGRATED`, `REPLACED`, `OBSOLETE`, `EXCLUDED_WITH_REASON`, `HUMAN_DECISION_REQUIRED`, or `UNVERIFIED`. The states `UNACCOUNTED`, `UNKNOWN_WITHOUT_REASON`, and `SILENTLY_OMITTED` are strictly forbidden.
