...
1. **Step 1: Recursive Inventory, Source Asset, Hook, Entity, Form & Configuration Discovery**
   ...
   - Dissect every custom PHP file, class, interface, trait, constructor, procedural hook implementation, custom entity type, field definition, revision mechanism, translation setting, form builder, form alter, AJAX callback, and configuration/state access.
   - Catalog discovered artifacts into the respective taxonomies (9-type hook taxonomy, 20-type configuration taxonomy, 26-target entity taxonomy, 19-target form/AJAX taxonomy).

2. **Step 2: Dependency, Caller Graph, Hook Invocation, Entity Reference & Config Lifecycle Mapping**
   - Identify core module, contrib module, custom module, entity reference, form dependency, and configuration/state dependencies using `skills/dependency-analysis`.
   - Map inter-file and inter-module calls to custom classes (`new ClassName()`, static calls), procedural functions, custom hook subscribers/callers, entity reference topologies, form callers, AJAX endpoints, and configuration reads/writes/deletes.
...
3. **Step 3: Business Behavior, Hook Dissection, Form & Entity/Configuration Decomposition**
   - Extract discrete functional units from custom PHP classes, `.inc` files, procedural hooks, custom entities, field definitions, form builders, and configuration forms.
...
4. **Step 4: Legacy API Audit, Constructor Inspection, Form API, Entity & State Analysis**
   - Audit constructors (`__construct()` and legacy `ClassName()`): extract parameters, global references (`$user`, `$conf`), and procedural calls (`variable_get()`, `db_query()`, `entity_load()`, `drupal_get_form()`).
   - Audit legacy Drupal 7 procedural APIs (`variable_get()`, `variable_set()`, `variable_del()`, `entity_load()`, `entity_save()`, `field_info_field()`, `EntityFieldQuery`, `drupal_get_form()`, `ajax_render()`, `ajax_command_*()`).
   - Distinguish Content Entities vs Config Entities vs Configuration (CMI) vs State (State API) vs Forms vs Cache.
...
5. **Step 5: Modern Architecture Design (Non-1:1 Mapping, Data Models, Entities, Forms, AJAX, CMI & PSR-4)**
...
6. **Step 6: Target-Ready Design Review, Constructor DI & Entity/Form Schema Verification**
